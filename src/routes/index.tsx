...
import { FamilyDetails } from "@/components/wedding/FamilyDetails";
...
const title = meta?.title ?? `${couple.brideShort} & ${couple.groomShort} — Wedding Invitation`;
const description =
  meta?.description ??
  `${couple.bride} and ${couple.groom} invite you to celebrate their wedding from ${event.dateLabel} at ${venue.name}, ${venue.address}.`;
const ogUrl = meta?.url ?? "/";
const ogImage = meta?.image ?? "/og-image.jpg";
const siteName = meta?.siteName ?? `${couple.brideShort} & ${couple.groomShort} Wedding`;
